...
typedef enum {
    ...
    REPLAY_TWEETS,
    REPLAY_WCUP,
    NUM_THROUGHPUT_CONTROL_MODES
} throughput_control_mode_t;
static char *throughput_control_mode_str[4] =
    {"throughput_file", "replay_adsb", "replay_tweets", "replay_wcup"};
/* Delivery report callbacks */
static void dr_msg_throughput_file_cb (rd_kafka_t *, const rd_kafka_message_t *, void *);
static void dr_msg_replay_cb (rd_kafka_t *, const rd_kafka_message_t *, void *);
static void (*dr_msg_cbs[NUM_THROUGHPUT_CONTROL_MODES])
    (rd_kafka_t*, const rd_kafka_message_t*, void *) = 
         {dr_msg_throughput_file_cb, dr_msg_replay_cb, dr_msg_replay_cb, dr_msg_replay_cb};
/* Read message functions */
static int read_msg (char *);
static int read_adsb_msg (char *);
static int read_tweets_msg (char *);
static int read_wcup_msg (char *);
static int (*read_msg_funcs[NUM_THROUGHPUT_CONTROL_MODES]) (char *) = 
    {read_msg, read_adsb_msg, read_tweets_msg, read_wcup_msg};

typedef struct {
    char *brokers;              /* CSV list of brokers (host:port) */
    char *topic;
    char *batch_num_messages;
    throughput_control_mode_t throughput_control_mode;
...
static int run = 1;
static long time_start = 0, time_last_produce = 0, time_last_check = 0, time_last_print_status = 0;
...
void print_config(producer_conf_t *conf) {
    ...
    printf("\tbatch_num_messagess=%s\n", conf->batch_num_messages);
    printf("\tthroughput_control_mode=%s\n", throughput_control_mode_str[conf->throughput_control_mode]);
...
int parse_config(void* user, const char* section, const char* name, const char* value) {
    ...
    else if (MATCH("throughput", "control_mode")) {
        if (strcasecmp(value, "throughput_file")) 
            conf->throughput_control_mode = THROUGHPUT_FILE;
        else if (strcasecmp(value, "replay_adsb"))
            conf->throughput_control_mode = REPLAY_ADSB;
        else if (strcasecmp(value, "replay_tweets"))
            conf->throughput_control_mode = REPLAY_TWEETS;
        else if (strcasecmp(value, "replay_wcup"))
            conf->throughput_control_mode = REPLAY_WCUP;
        else {
            fprintf(stderr, "%% Illegal throughput control option: %s\n", optarg);
            goto illegal_config;            
        }
    }
...
static int read_wcup_msg(char *buf) {
    return 0;
}

static 
void dr_msg_throughput_file_cb (rd_kafka_t *rk, const rd_kafka_message_t *rkmessage, void *opaque) {
    ...
    if (rkmessage->err)
        ...
        time_delta = time_curr - time_last_check;
        ...
        if (time_delta >= THROUGHPUT_CHECK_INTERVAL_MSEC) {
            ...
            if (throughput_target < throughput_curr) {
                ...
                throughput_adjusted = (float)bytes_sent / (get_current_time_msec() - time_last_check);
                printf("Throughput: target %.2f vs. adjusted %.2f bytes/s\n",
                       throughput_target, throughput_adjusted);
            }
            bytes_sent = 0;
            msgs_sent = 0;
            time_last_check = time_curr;
        }
    }
...
static 
void dr_msg_replay_cb (rd_kafka_t *rk, const rd_kafka_message_t *rkmessage, void *opaque) {
}
...
int main (int argc, char** argv) {
    ...
    time_start = time_last_check = get_current_time_msec();
    
    /* Open throughput and message files */
    if (prod_conf.throughput_control_mode == THROUGHPUT_FILE &&
        (throughput_fp = fopen(prod_conf.throughput_file, "r")) == NULL) {
    ...
    rd_kafka_conf_set_dr_msg_cb(kafka_conf, dr_msg_cbs[prod_conf.throughput_control_mode]);
    ...
    while (run) {
        size_t len = read_msg_funcs[prod_conf.throughput_control_mode](msgbuf);
        if (len == 0) {
        ...
        #if 0
        time_curr = get_current_time_msec();        
        if (time_curr - time_last_print_status > STATUS_PRINT_INTERVAL_SEC * 1000) {
            printf("time=%ld, msgs_sent_total=%d, bytes_sent_total=%ld\n", 
                   time_curr, msgs_sent_total, bytes_sent_total);
            time_last_print_status = time_curr;
        }
        #endif
